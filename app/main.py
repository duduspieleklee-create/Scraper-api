    ...
    try:
        ...
        for search, ad_count in top_rows.all():
            top_searches_dict.append({
                ...
                "keyword": search.keyword, "ad_count": int(ad_count or 0)
            })

        price_row = await db.execute(select(
            func.min(SeenAd.price).label("mn"), func.max(SeenAd.price).label("mx"),
            func.avg(SeenAd.price).label("av"), func.count(SeenAd.price).label("cnt")
        ...
        for search, ad_count in rows.all():
            searches_dict.append({
                "id": search.id, "name": search.name,
                "keyword": search.keyword, "enabled": search.enabled,
                "ad_count": int(ad_count or 0), "created_at": search.created_at
            })
        ...
        recent_ads_dict = [
            {
                "id": ad.id, "title": ad.title, "price": ad.price,
                "link": ad.link, "first_seen": ad.first_seen, "ad_id": ad.ad_id,
            }
        ...
        return templates.TemplateResponse(
            ...
            context={
                "uptime_str": uptime_str, "pid": pid, "db_ok": db_ok,
                "worker_online": worker_online, "worker_last_seen_str": worker_last_seen_str,
                "cpu": cpu, "memory_percent": mem.percent,
                "memory_used_gb": round(mem.used / (1024 ** 3), 1),
                "memory_total_gb": round(mem.total / (1024 ** 3), 1),
                "disk_percent": disk.percent,
                "disk_used_gb": round(disk.used / (1024 ** 3), 1),
                "disk_total_gb": round(disk.total / (1024 ** 3), 1),
                "total_searches": total_searches, "active_searches": active_searches,
                "paused_searches": paused_searches, "total_ads": total_ads,
                "ads_24h": ads_24h, "ads_per_day": ads_per_day, "total_users": total_users,
                "active_proxies": active_proxies, "total_proxies": total_proxies,
                "proxy_list": proxy_list, "tokens_spent": tokens_spent,
                "tokens_24h": tokens_24h, "webhook_ok": webhook_ok,
                "webhook_fail": webhook_fail, "top_searches": top_searches_dict,
                "price_min": price_min, "price_max": price_max,
                "price_avg": price_avg, "price_count": price_count,
                "searches_with_counts": searches_dict, "recent_ads": recent_ads_dict,
                "now": now, "error": None
            }
        )
        ...
        return templates.TemplateResponse(
            request=request, name="dashboard.html",
            context={
                "error": str(exc), "uptime_str": "—", "pid": "—",
                "db_ok": False, "worker_online": False, "worker_last_seen_str": None,
                "cpu": 0, "memory_percent": 0,
                "memory_used_gb": 0, "memory_total_gb": 0,
                "disk_percent": 0, "disk_used_gb": 0, "disk_total_gb": 0,
                "total_searches": 0, "active_searches": 0, "paused_searches": 0,
                "total_ads": 0, "ads_24h": 0, "ads_per_day": [],
                "total_users": 0, "active_proxies": 0, "total_proxies": 0,
                "proxy_list": [], "tokens_spent": 0, "tokens_24h": 0,
                "webhook_ok": 0, "webhook_fail": 0, "top_searches": [],
...
async def scrape(url: str = Query(..., description="URL zum Scrapen")):
    ...
    try:
        ...
        result = {
            ...
            "status": resp.status_code, "timestamp": datetime.utcnow().isoformat()
        }
